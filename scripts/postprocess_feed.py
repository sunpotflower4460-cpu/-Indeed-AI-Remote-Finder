#!/usr/bin/env python3
"""Post-process the refreshed feed into a stable rolling candidate pool.

- Drop rows that explicitly contradict full-remote work.
- Collapse duplicate postings with the same normalized company/title.
- Keep recently seen viable jobs for up to 14 days as REVIEW-only reserve rows.
- Prefer fresh/live rows, but do not let one sparse refresh empty the app.
- Cap the visible pool at 80 candidates.
"""
...
POOL_RETENTION_MAX = timedelta(days=14)
...
def carryover_rows(current: list[dict], previous: list[dict], now: datetime) -> list[dict]:
    """Retain missing candidates as lower-ranked reserve rows for up to 14 days.

    last_seen is deliberately not advanced. A row therefore ages out even if it
    keeps being carried by every refresh.
    """
    ...
    for old in previous:
        ...
        if missing_for > POOL_RETENTION_MAX:
            continue
        ...
        row["carryover_reason"] = "最新スキャンでは未再検出。直近候補として最大14日だけ保持"
        row["freshness_confidence"] = min(
            int(row.get("freshness_confidence") or 0),
            max(35, 60 - days_missing * 3),
        )
        row["score"] = min(int(row.get("score") or 0), max(48, 74 - days_missing * 2))
        carried.append(row)
    return carried


def combined_sort_key(row: dict) -> tuple[int, int, int, int, int, float]:
    first_seen = parse_iso(row.get("first_seen"))
    first_seen_ts = first_seen.timestamp() if first_seen else 0.0
    return (
...
def process(current_payload: dict, previous_payload: dict | None = None) -> dict:
    ...
    combined = combined[:POOL_TARGET_MAX]

    current_payload["jobs"] = combined
    ...
    current_payload["live_jobs"] = sum(1 for row in combined if str(row.get("id") or "") in live_ids)
    current_payload["new_jobs"] = sum(1 for row in combined if str(row.get("id") or "") not in previous_ids and not row.get("carryover"))
    current_payload["candidate_pool_size"] = len(combined)
    current_payload["pool_target_min"] = int(current_payload.get("pool_target_min") or POOL_TARGET_MIN)
    current_payload["pool_target_max"] = int(current_payload.get("pool_target_max") or POOL_TARGET_MAX)
    current_payload["pool_under_target"] = len(combined) < current_payload["pool_target_min"]
...
def main() -> None:
    ...
    print(
        f"postprocessed pool={len(result.get('jobs', []))}; "
        f"live={result.get('live_jobs', 0)} new={result.get('new_jobs', 0)} "
        f"reserve={result.get('carryover_jobs', 0)}; "
