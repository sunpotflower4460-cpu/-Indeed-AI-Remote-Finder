#!/usr/bin/env python3
"""Build a small, conservative candidate feed from public search-index results.

Important: this script does NOT request or scrape Indeed job pages. It only reads
public Bing RSS search results that point to Indeed, then ranks the title/snippet.
The final job status must be confirmed by the user on Indeed before applying.
"""
...
QUERIES = [
    ("data", 'site:jp.indeed.com/viewjob ("完全在宅" OR "フルリモート") "データ入力"'),
    ("annotation", 'site:jp.indeed.com/viewjob ("完全在宅" OR "フルリモート") (アノテーション OR "AIトレーナー")'),
    ("writing", 'site:jp.indeed.com/viewjob ("完全在宅" OR "フルリモート") (校正 OR 要約 OR ライティング)'),
    ("transcription", 'site:jp.indeed.com/viewjob ("完全在宅" OR "フルリモート") (文字起こし OR transcription)'),
    ("research", 'site:jp.indeed.com/viewjob ("完全在宅" OR "フルリモート") (リサーチ OR research)'),
    ("ec", 'site:jp.indeed.com/viewjob ("完全在宅" OR "フルリモート") (商品登録 OR EC OR Shopify)'),
    ("office", 'site:jp.indeed.com/viewjob ("完全在宅" OR "フルリモート") (メール OR チャット OR "事務")'),
    ("translation", 'site:jp.indeed.com/viewjob ("完全在宅" OR "フルリモート") (翻訳 OR translation)'),
    ("ai_eval", 'site:jp.indeed.com/viewjob ("在宅" OR remote) ("AI評価" OR "データ評価" OR "品質評価")'),
    ("classification", 'site:jp.indeed.com/viewjob ("在宅" OR remote) (分類 OR タグ付け OR labeling)'),
    ("moderation", 'site:jp.indeed.com/viewjob ("完全在宅" OR "フルリモート") (モデレーション OR moderation)'),
    ("spreadsheet", 'site:jp.indeed.com/viewjob ("完全在宅" OR "フルリモート") (Excel OR スプレッドシート OR 集計)'),
]

REMOTE_STRONG = {
    "完全在宅": 74, "フルリモート": 74, "完全リモート": 74,
    "fully remote": 74, "100% remote": 74, "全国どこからでも": 68,
    "勤務地自由": 60,
}
REMOTE_MEDIUM = {
...
AUTO_STRONG = {
    ...
    "分類": 26, "要約": 28, "校正": 27, "商品登録": 27,
    "データ整理": 27, "データ収集": 24, "データチェック": 27,
    "品質チェック": 24, "品質評価": 24, "リスト作成": 25,
    "定型": 22, "転記": 30, "入力業務": 28,
}
AUTO_MEDIUM = {
    ...
    "csv": 14, "shopify": 16, "ec": 9, "canva": 10,
    "モデレーション": 18, "moderation": 18, "レビュー": 10,
}
...
TAG_RULES = [
    ("完全リモート", ["完全在宅", "フルリモート", "完全リモート", "fully remote", "100% remote", "全国どこからでも"]),
    ("データ", ["データ入力", "データ整理", "データ収集", "転記", "集計", "csv"]),
    ("AI評価", ["アノテーション", "annotation", "aiトレーナー", "ai trainer", "ai評価", "データ評価", "分類", "タグ付け", "labeling"]),
    ("文章", ["文字起こし", "transcription", "翻訳", "translation", "校正", "要約", "ライティング", "記事作成"]),
    ("リサーチ", ["リサーチ", "research"]),
    ("事務", ["事務", "メール", "チャット", "excel", "スプレッドシート"]),
    ("EC", ["商品登録", "shopify", "ec"]),
]
...
def is_indeed_job_url(link: str) -> bool:
    ...
    if not (host == "indeed.com" or host.endswith(".indeed.com")):
        return False
    return ("/viewjob" in parsed.path.lower()) or ("jk=" in parsed.query.lower())
...
def score_job(text: str, published: datetime | None, previous: dict | None) -> Scores:
    ...
    return Scores(
        ...
        overall=overall,
        tier=tier,
        remote_reasons=remote_reasons[:5],
        automation_reasons=automation_reasons[:7],
...
def tags_for(text: str) -> list[str]:
    ...
    for label, keys in TAG_RULES:
        if any(key.lower() in t for key in keys):
            tags.append(label)
...
def fetch(query: str) -> bytes:
    url = "https://www.bing.com/search?" + urllib.parse.urlencode(
        {"q": query, "format": "rss", "setlang": "ja-JP", "count": "20"}
    )
    request = urllib.request.Request(
        url,
        headers={
            "User-Agent": "Mozilla/5.0 (compatible; AI-Remote-Finder/2.0; public-search-index-only)",
...
def main() -> None:
    ...
    def fetch_one(pair):
        category, query = pair
        return category, query, ET.fromstring(fetch(query))
    ...
    for category, query, root in results:
        for item in root.findall(".//item"):
            ...
            if not title or not is_indeed_job_url(link):
                continue
            if title.lower() in {"求人", "indeed", "求人検索"}:
                continue
    ...
    if query_success == 0:
        print("ERROR: every search query failed; keeping previous feed unchanged", file=sys.stderr)
        raise SystemExit(2)
    ...
    payload = {
        ...
        "errors": errors[:8],
        "method": "public-search-index-only",
        "jobs": jobs,
    }
    ...
    print(f"wrote {len(jobs)} jobs ({high} high / {review} review) to {OUT}; queries {query_success}/{len(QUERIES)}")
