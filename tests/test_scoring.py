...
class ScoringGuardTests(unittest.TestCase):
    ...
    def test_vague_remote_role_does_not_pass(self):
        self.assertNotEqual(s("完全在宅 一般スタッフ 未経験歓迎").tier, "high")
    ...
    def test_api_remote_filter_only_is_review(self):
        self.assertEqual(s("データ入力 転記 データ整理", remote_filter=True).tier, "review")

    def test_old_listing_is_review(self):
        self.assertEqual(s("完全在宅 データ入力 転記 データ整理", days_old=20).tier, "review")

    def test_very_old_listing_is_hidden(self):
        self.assertEqual(s("完全在宅 データ入力 転記 データ整理", days_old=45).tier, "hidden")

    def test_customer_support_not_high(self):
        self.assertNotEqual(s("完全在宅 データ入力 転記 カスタマーサポート").tier, "high")

    def test_physical_task_hidden(self):
        self.assertEqual(s("完全在宅 商品登録 データ入力 梱包 商品撮影").tier, "hidden")

    def test_relative_japanese_date(self):
        now=datetime(2026,8,19,tzinfo=timezone.utc)
        self.assertEqual(mod.parse_relative_posted_at("8日前",now),now-timedelta(days=8))

    def test_relative_english_date(self):
        now=datetime(2026,8,19,tzinfo=timezone.utc)
        self.assertEqual(mod.parse_relative_posted_at("3 days ago",now),now-timedelta(days=3))

    def test_30_plus_is_stale(self):
        now=datetime(2026,8,19,tzinfo=timezone.utc)
        self.assertLessEqual(mod.parse_relative_posted_at("30+ days ago",now),now-timedelta(days=31))
    ...
    def test_build_row_requires_indeed_apply(self):
        job={"title":"完全在宅 データ入力 転記","description":"データ整理と転記。完全在宅。","detected_extensions":{"posted_at":"1 day ago"},"apply_options":[{"title":"Company","link":"https://example.com/apply"}]}
        self.assertIsNone(mod.build_row(job,"structured",{}))

    def test_build_row_accepts_indeed_apply(self):
        job={"title":"完全在宅 データ入力 転記","company_name":"Example","location":"Anywhere","description":"データ整理、転記、データ入力。フルリモート。","detected_extensions":{"posted_at":"1 day ago"},"apply_options":[{"title":"Indeed","link":"https://jp.indeed.com/viewjob?jk=job123&utm_source=x"}]}
