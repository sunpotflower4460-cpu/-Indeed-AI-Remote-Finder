...
class RemoteFeedValidationTests(unittest.TestCase):
    ...
    def test_remote_query_only_review_requires_visible_confirmation_tag(self):
        good = row(
            tier="review",
        ...
        self.assertEqual(mod.validate({"jobs": [good]}), [])

        bad = row(
            tier="review",
            reasons=["Google Jobs:在宅勤務フィルタ"],
            tags=["データ"],
            remote_search_only=True,
        )
        errors = mod.validate({"jobs": [bad]})
        self.assertTrue(any("在宅要確認" in value for value in errors))

    def test_remote_query_only_can_never_be_high(self):
        errors = mod.validate({"jobs": [row(remote_search_only=True, tags=["在宅要確認"])]})
