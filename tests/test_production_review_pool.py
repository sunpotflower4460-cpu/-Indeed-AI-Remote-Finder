...
def scores(*, automation=40, remote=68, risk=0, reasons=None, remote_reasons=None):
    return mod.acquisition.legacy.Scores(
        ...
        automation_reasons=list(reasons or ["データ入力"]),
        risk_reasons=[],
    )


class ProductionReviewPoolTests(unittest.TestCase):
    def test_clear_automation_signal_can_enter_next_best_review(self):
        self.assertTrue(
            mod.production_review_fallback(scores(), datetime.now(timezone.utc))
        )

    def test_weak_automation_is_not_kept_just_to_pad_count(self):
        self.assertFalse(
            mod.production_review_fallback(
                scores(automation=22, reasons=["事務"]),
    ...
    def test_meaningful_human_dependency_risk_is_not_kept(self):
        self.assertFalse(
            mod.production_review_fallback(
                scores(automation=90, risk=30), datetime.now(timezone.utc)
            )
        )
