...
class AcquisitionQualityTests(unittest.TestCase):
    def test_weekly_partial_remote_is_blocked(self):
        found = mod.partial_remote_blockers(
            job("慣れたら在宅勤務週1～2日。残りは出社です。")
        )
    ...
    def test_explicit_partial_remote_phrase_is_blocked(self):
        self.assertIn(
            "在宅あり",
            mod.partial_remote_blockers(job("在宅ありのハイブリッド求人です")),
        )

    def test_negated_hybrid_is_not_blocked(self):
        self.assertEqual(
            mod.partial_remote_blockers(
                job("完全在宅です。ハイブリッド勤務は不可、出社不要です。")
            ),
            [],
        )

    def test_coordination_core_work_is_attention_blocked(self):
        found = mod.quality_attention_blockers(
            job("社内外関係者との調整業務と進捗管理を担当します")
        )
    ...
    def test_negated_coordination_is_not_attention_blocked(self):
        self.assertEqual(
            mod.quality_attention_blockers(job("データ入力中心。調整業務なし。")),
            [],
        )
    ...
    def test_review_quality_floor_is_enforced(self):
        self.assertTrue(
            mod.review_row_meets_quality(
                {"automation_confidence": 55, "human_dependency_risk": 25}
            )
        )
        self.assertFalse(
            mod.review_row_meets_quality(
                {"automation_confidence": 54, "human_dependency_risk": 0}
            )
        )
        self.assertFalse(
            mod.review_row_meets_quality(
                {"automation_confidence": 90, "human_dependency_risk": 26}
            )
        )
