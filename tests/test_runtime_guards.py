...
class RuntimeGuardTests(unittest.TestCase):
    ...
    def test_production_uses_work_from_home_adapter_without_promoting_it_to_high(self):
        workflow = (ROOT / ".github" / "workflows" / "update-jobs.yml").read_text(encoding="utf-8")
        ...
        self.assertIn("remote_search_only", adapter)
        self.assertIn("REMOTE_EXPLICIT_FULL", adapter)
        self.assertIn("DAILY_APPLICATION_TARGET = 10", acquisition)
        self.assertIn("DISPLAY_TARGET = 30", acquisition)
        self.assertIn("POOL_LIMIT = 100", acquisition)
        self.assertIn("MAX_REQUESTS_PER_RUN = 18", acquisition)
    ...
    def test_recommendation_queue_defaults_to_all_hides_applied_and_tracks_daily_ten(self):
        ...
        self.assertIn("todayAppliedCount", app)
