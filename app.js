...
function currentRows(){
  ...
  let rows=state.jobs.filter(j=>{
    ...
    if(state.mode==='dual')return !isHidden&&hasDualPass(j);
...
function render(){
  const rows=currentRows();$('#resultCount').textContent=`${rows.length}件`;
  const names={high:'高確度だけ',dual:'LLM二重審査済み',review:'要確認候補',all:'全候補',saved:'保存済み',applied:'応募済み',hidden:'非表示'};$('#listTitle').textContent=names[state.mode]||'求人候補';
  if(!rows.length){$('#jobs').innerHTML=`<div class="empty"><b>この条件では候補がありません。</b><br>品質を落として無理に表示していません。<br><span>${state.mode==='dual'?'LLM審査はOPENAI_API_KEY設定後、新規・変更求人から順次付きます。':'「要確認候補」か「Indeedで今すぐ検索」を使えます。'}</span></div>`;return;}
  $('#jobs').innerHTML=rows.map(j=>{
    const saved=state.saved.has(j.id),hidden=state.hidden.has(j.id),applied=state.applied.has(j.id),tier=effectiveTier(j),high=tier==='high',dual=hasDualPass(j),published=j.search_published_at||j.last_seen,org=[j.company,j.location].filter(Boolean).map(esc).join(' · ');
    return `<article class="card ${high?'high':''} ${dual?'dualCard':''} ${tier==='expired'?'expired':''}"><div class="cardHead"><div class="verdict ${dual?'dualBadge':high?'ok':tier==='expired'?'expiredBadge':'review'}">${dual?'◎ 二重審査通過':verdictText(tier)}</div><div class="fresh">${esc(ageLabel(published))}</div></div><h3>${esc(j.title)}</h3>${org?`<div class="org">${org}</div>`:''}<p class="snippet">${esc(j.snippet||'求人概要なし')}</p><div class="scores"><div><span>AI代替</span><b class="${meterClass(j.automation_confidence)}">${j.automation_confidence??'-'}</b></div><div><span>完全在宅</span><b class="${meterClass(j.remote_confidence)}">${j.remote_confidence??'-'}</b></div><div><span>新しさ</span><b class="${meterClass(j.freshness_confidence)}">${j.freshness_confidence??'-'}</b></div></div><div class="why">${esc(reasonText(j))}</div><div class="tags">${(j.tags||[]).slice(0,5).map(t=>`<span class="tag">${esc(t)}</span>`).join('')}${j.seen_count>1?`<span class="tag repeat">再検出 ${j.seen_count}回</span>`:''}${j.duplicate_count>1?`<span class="tag">類似掲載 ${j.duplicate_count}件を統合</span>`:''}${j.carryover?'<span class="tag carry">今回未再検出</span>':''}${dual?'<span class="tag dualTag">LLM二重審査 ✓</span>':''}${applied?'<span class="tag appliedTag">応募済み</span>':''}</div>${j.risk_reasons?.length?`<div class="risk">要注意: ${esc(j.risk_reasons.join('・'))}</div>`:''}${llmPanel(j)}${tier==='expired'?'<div class="risk">掲載日から30日を超えたため通常一覧から除外しています。リンク先が生きていても再確認してください。</div>':''}<div class="actions"><button class="btn ghost save" data-id="${esc(j.id)}">${saved?'★ 保存済み':'☆ 保存'}</button><button class="btn ghost applied" data-id="${esc(j.id)}">${applied?'✓ 応募済み':'○ 応募済みにする'}</button><button class="btn ghost hide" data-id="${esc(j.id)}">${hidden?'↩ 戻す':'× 非表示'}</button><a class="btn primary" href="${esc(j.url)}" target="_blank" rel="noopener noreferrer">求人を確認・応募 →</a></div><div class="meta">掲載目安: ${esc(j.posted_label||fmtDate(j.search_published_at))} / 最終検出: ${esc(fmtDate(j.last_seen))}${j.llm_model?` / LLM: ${esc(j.llm_model)}`:''}</div></article>`;
  }).join('');
...
function updateHealth(d){const h=feedAgeHours();let text=d.provider_configured===false?'求人自動更新はSERPAPI_KEY設定後に有効':d.query_total?`自動検索 ${d.query_success||0}/${d.query_total}・Indeed応募URL ${d.indeed_apply_jobs??'-'}件`:'自動検索 -';if(d.llm_provider_configured===false)text+=' / LLM二次審査はOPENAI_API_KEY未設定';else if(d.llm_provider_configured===true)text+=` / LLM審査 ${d.llm_reviewed_jobs||0}件・二重通過 ${d.llm_strict_jobs||0}件`;if(Number.isFinite(h)&&h>24)text+=` / データ更新から${Math.floor(h)}時間`;$('#sourceHealth').textContent=text;if(d.provider_configured===false||d.llm_provider_configured===false||(d.query_success||0)<(d.query_total||0)||(Number.isFinite(h)&&h>24))$('#sourceHealth').classList.add('warnText');}
async function boot(){try{const r=await fetch(`./data/jobs.json?v=${Date.now()}`,{cache:'no-store'});if(!r.ok)throw new Error(`HTTP ${r.status}`);const d=await r.json();state.jobs=d.jobs||[];state.meta=d;const high=state.jobs.filter(j=>effectiveTier(j)==='high').length,review=state.jobs.filter(j=>effectiveTier(j)==='review').length;$('#countHigh').textContent=high;$('#countReview').textContent=review;$('#updated').textContent=fmtDate(d.generated_at);updateHealth(d);render();}catch(e){$('#jobs').innerHTML='<div class="empty"><b>求人データを読み込めませんでした。</b><br>下の「Indeedで今すぐ検索」は利用できます。</div>';$('#countHigh').textContent='0';$('#countReview').textContent='0';$('#updated').textContent='-';}}
