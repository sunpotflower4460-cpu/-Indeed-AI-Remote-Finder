...
- No explicit requirement for continuous human presence such as 常時ログイン, 常時オンライン, オンライン待機, real-time/live monitoring, immediate-response SLAs, or a requirement to remain online throughout a shift.
- A fixed working schedule by itself is **not** a continuous-presence blocker. Scheduled or batch automation can run at fixed hours; rejection requires evidence that a human must remain present/available while work arrives.
- Explicit negations such as 常時ログイン不要 / オンライン待機なし / no need to stay online are scrubbed before the presence test so they do not create false rejections.
...
- LLM blockers that explicitly describe real-time waiting, immediate response, standby/on-call, or always-online requirements are treated as continuous-presence mismatches even if the nominal automation percentage is high.
...
The PWA uses `candidateCacheV3`. The cache generation changed with quality policy v2 so old v1 candidates are not silently resurfaced on iPhone. Cached rows are rechecked against v2 thresholds before they can appear as local reserve candidates. The server-side final presence gate runs on every refreshed feed, including carried reserve rows; a future client cache generation bump can additionally require the presence stamp locally once enough live v2+presence stock exists.
